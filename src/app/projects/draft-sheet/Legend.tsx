...
/**
 * How to read a row — the long form.
 *
 * The key used to carry its own header, its own open/closed state and the
 * platform checkboxes. All three moved to `BoardStrip`, which is where a
 * returning reader looks now; what is left here is the part that is genuinely
 * a reference page: every glyph on a row, spelled out.
 *
 * Still worth reading once and in the way afterwards, so it stays collapsed by
 * default. Opened it is around 700px tall, which on a 390px phone is the entire
 * screen ahead of the board.
 */
