// Persona prompt construction — TS port of projects/religious-voices/server/persona.py.
//
// Three-block structure for Bedrock Converse prompt caching:
//   Block 1 (STABLE)     voice / tagging rules / worked examples. Shared
//                        across leaders; cached so subsequent calls (even
//                        to different leaders) hit the same prefix.
//   Block 2 (PER-LEADER) identity card (name, dates, themes). Cached for
//                        the duration of the user's session on this leader.
//   Block 3 (PER-TURN)   retrieved passages + the visitor question. Lives
//                        in the human message; not cached.
//
// Bedrock Converse marks cache boundaries with `cachePoint` system blocks
// — distinct from Anthropic-native `cache_control: {type: "ephemeral"}`.
// Empirical note from the v2 implementation: Sonnet's minimum cacheable
// prefix is ~2100 tokens, not the 1024 the docs cite. STABLE_BLOCK is
// padded past that threshold intentionally; if cache_read_input_tokens
// drops to zero after a trim, the trim went below the floor.

import type { Leader } from "./types";

export const STABLE_BLOCK = `You are roleplaying as a specific religious leader, identified in the
...
export function buildLeaderBlock(leader: Leader): string {
...
export function buildUserMessage(query: string, retrieved: RetrievedPassage[]): string {
  const passages =
    retrieved.length > 0
      ? retrieved
          .map(
            (r, i) =>
              `[${i + 1}] ${r.work_title}${r.year ? `, ${r.year}` : ""}\n${r.text.trim()}`,
          )
          .join("\n\n")
