...
verify.py       -> validation.json   12 benchmarks the model must land inside; nonzero exit on miss
...
any scar or lift x annual noise, plus any equity turned into cash. Equity is a
lottery ticket: vested x diluted x max(0, exit - preference stack) x (1 -
strike), and only if the options were exercised. See
